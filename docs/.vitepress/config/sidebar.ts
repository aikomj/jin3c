...
export const sidebar: DefaultTheme.Config['sidebar'] = {
  '/categories/fragments/': getItemsByYear("categories/fragments"),
  '/categories/issues/': getItemsByYear("categories/issues"),
  '/categories/solutions/': getItemsByYear("categories/solutions"),
  '/categories/tools/': getItems("categories/tools"),
  '/categories/docker/': getItems("categories/docker"),
  '/categories/microservice/': getItems("categories/microservice"),

  '/courses/design-mode/': getItems("courses/design-mode"),
  '/courses/elasticsearch/': getItems("courses/elasticsearch"),
  '/courses/java/': getItems("courses/java"),
  '/courses/mq/': getItems("courses/mq"),
  '/courses/mybatis/': getItems("courses/mybatis"),
  '/courses/mysql/': getItems("courses/mysql"),
  '/courses/redis/': getItems("courses/redis"),
}
...
function getItemsByYear (path: string) {
  ...
  sync(`docs/${path}/*`, {
    ...
    // 添加年份分组
    yearGroups.unshift({
      text: `<img class="chinese-zodiac" style="position: static; vertical-align: middle; padding-bottom: 3px;" src="/jin3c/img/svg/chinese-zodiac/${getChineseZodiac(year.replace('年', ''))}.svg" title="${getChineseZodiacAlias(year.replace('年', ''))}" alt="生肖">
            ${year}年 (${articleItems.length}篇)`,
      items: articleItems,
      collapsed: true,
    });
